...
# Funksjon for formatering av respons som sendes til klienter
# Returnerer en statuskode (inspirert av HTTP) og eventuelle data.
# Statuskoder:
#	201 - JOIN OK: Join OK,
#	211 - HAND OK: Sender hånd.
#	221 - SHOWDOWN OK: Sender resultat av poker showdown
#	222 - SHOWDOWN WIN: Sendes ut hvis klient også er vinner av showdown
#	223 - SHOWDOWN LOSE: Sendes ut hvis klient ikke vant showdown

#	400 - Bad request: Forespørselen kunne ikke behandles.
#	401 - JOIN FAIL FULL: Bordet er fullt
#	402 - JOIN FAIL DUP: Spiller allerede registrert
#	411 - HAND FAIL: Ikke registrert spiller
#	412 - HAND FAIL: Kan ikke få hånd før andre spillere er klare
#	413 - HAND FAIL DUP: Spilleren har allerede fått en hånd utlevert
#	421 - SHOWDOWN FAIL: Spillere har ikke fått utlevert hendene ennå
#	422 - SHOWDOWN FAIL: Ikke registrert spiller
def resformat(statuscode, data):
	return str(statuscode) + "," + str(data)
			...
			if data:
				...
				if data:
					...
					if data == 'JOIN\n':
						...
						if not s.getpeername() in players:
							if len(players) < seats:
								print "FIKK JOIN"
								data = "Join OK"
								players.append(s.getpeername())
								message_queues[s].put(str(resformat(201, data)))						
							else:
								data = "Beklager, bordet er fullt"
								message_queues[s].put(str(resformat(401, data)))
						else:
							data = "Du er allerede registrert"
							message_queues[s].put(str(resformat(402, data)))
					

					# HAND
					elif data == 'HAND\n':
						
						if s.getpeername() in players:
							for player in players:
								if player == s.getpeername():
									if len(players) == seats:
										if not player in playerhands:
											print "FIKK HAND, nok spillere tilstede og ikke duplikat"
											data = ','.join(str(x) for x in hands[(handsdelt)])
											playerhands[s.getpeername()] = data
											handsdelt += 1
											message_queues[s].put(str(resformat(211, data)))
										else:
											print "FIKK HAND, spiller har allerede fått hand"
											data = "Hand allerede utgitt"
											message_queues[s].put(str(resformat(412, data)))
									else:
										print "FIKK HAND, ikke nok spillere"
										data = "Vent til bordet er fylt opp"
										message_queues[s].put(str(resformat(412, data)))

						else:
							print "FIKK HAND, Ikke registrert spiller"
							data = "Ikke registrert spiller"
							message_queues[s].put(str(resformat(412, data)))
					...
					#SHOWDOWN
						...
						if s.getpeername() in players:
							if handsdelt == seats:
								...
								winner_hand = str(poker.poker(hands, "winner"))
								winner_hand = winner_hand.replace("'", "")
								winner_hand = winner_hand.replace("[", "")
								winner_hand = winner_hand.replace("]", "")
								winner_hand = winner_hand.replace(" ", "")
								score = str(poker.poker(hands, "score"))
								data = "{},{}".format(score, winner_hand)
								print "SHOWDOWN DEBUG"
								print playerhands[s.getpeername()]
								print data

								message_queues[s].put(str(resformat(221, data)))

								# if playerhands[s.getpeername()] == data:
								# 	data = "Du vant, gratulerer!"
								# 	message_queues[s].put(str(resformat(222, data)))

								# else:
								# 	data = "Du tapte, beklager!"
								# 	message_queues[s].put(str(resformat(223, data)))

							else:
								print "FIKK SHOWDOWN, ikke alle spillere har fått kort ennå"
								data = "Vent til alle hender er delt ut"
								message_queues[s].put(str(resformat(421, data)))

						else:
							print "FIKK SHOWDOWN, Ikke registrert spiller"
							data = "Kun registrerte spillere kan se resultatet"
							message_queues[s].put(str(resformat(422, data)))

					# ELSE
					elif data != 'JOIN\n' or data != 'HAND\n' or data != 'SHOWDOWN\n':
						print data
						data = "Kunne ikke behandle foresporselen"
						message_queues[s].put(str(resformat(400, data)))
